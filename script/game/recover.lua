...
function yun_forceheal_exec()
    ...
    local l = wait_line("yun forceheal", 300, {StopEval=true}, 10, "^你(?:缓步行走多时|运功良久)，自觉经脉顺畅，内伤尽去，(?:在东方乙木之位停下了步伐|神元气足地站了起来)。$|"..
                                                                   "^你只觉(?:内息运转顺畅|体内【毒龙大法】运转正常)，将真气还合丹田，站起身来。$|"..
                                                                   "^你催运(?:摩天云气|紫霞功)疗伤(?:片刻，体内真气|多时，一道真气在体内)运转无碍，内伤已经平复，当即站起身来。$|"..
                                                                   "^你正(?:以奇门步法调匀真气|在运功疗伤)，忽觉内息後继乏力，祗得(?:停下脚步，|)暂缓疗伤(?:|，站起身来)。$|"..
                                                                   "^你渐感真气(?:不纯，后劲不继|不足，体内说不出的难受)，不得不将在体内搬运的内息收回。$|"..
                                                                   "^你没有内伤。$|"..
                                                                   "^你无法在战斗中运功疗伤。$|"..
                                                                   "^你现在正忙着呢。|"..
                                                                   "^\\( 你上一个动作还没有完成，不能施用内功。\\)$|"..
    ...
    elseif string.match(l[0], "化解十分困难") then
        return 1,"治疗失败"
    else
        if run_hp() < 0 then
            return -1
...
function yun_heal_exec()
    ...
    l = wait_line("yun heal", 300, {StopEval=true}, 10, "^你(?:缓步行走多时|运功良久)，自觉经脉顺畅，内伤尽去，(?:在东方乙木之位停下了步伐|神元气足地站了起来)。$|"..
                                                        "^你只觉(?:内息运转顺畅|体内【毒龙大法】运转正常)，将真气还合丹田，站起身来。$|"..
...
function dazuo_exec(target)
    ...
    elseif l[0] == "你现在正忙着呢。" or 
        ...
        wait(0.1)
        return dazuo_exec(target)
    elseif l[0] == "战斗中不能练内功，会走火入魔。" then
        if break_event() == true then
            return 1
        end
        if wait_no_fight() < 0 then
