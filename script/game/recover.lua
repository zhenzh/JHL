...
function yun_bidu(layer)
    message("info", debug.getinfo(1).source, debug.getinfo(1).currentline,
            "函数［ yun_bidu ］参数：layer = "..tostring(layer))
...
function yun_bidu_return(rc, msg)
    message("info", debug.getinfo(1).source, debug.getinfo(1).currentline,
            "函数［ yun_bidu_return ］参数：rc = "..tostring(rc)..", msg = "..tostring(msg))
    if var.yun_bidu == nil then
        return rc,msg
    end
    var.yun_bidu = nil
...
function yun_bidu_exec()
    ...
    local l = wait_line("yun bidu",
                        300, {StopEval=true}, 10,
...
function yun_forceheal(layer)
    message("info", debug.getinfo(1).source, debug.getinfo(1).currentline,
            "函数［ yun_forceheal ］参数：layer = "..tostring(layer))
...
function yun_forceheal_return(rc, msg)
    message("info", debug.getinfo(1).source, debug.getinfo(1).currentline,
            "函数［ yun_forceheal_return ］参数：rc = "..tostring(rc)..", msg = "..tostring(msg))
    if var.yun_forceheal == nil then
        return rc,msg
    end
...
function yun_forceheal_exec()
    ...
    local l = wait_line("yun forceheal",
                        300, {StopEval=true}, 10,
                        "^你没有内伤。$|"..
                        "^你无法在战斗中运功疗伤。$|"..
                        "^你现在正忙着呢。|"..
...
function yun_heal(layer)
    message("info", debug.getinfo(1).source, debug.getinfo(1).currentline,
            "函数［ yun_heal ］参数：layer = "..tostring(layer))
...
function yun_heal_return(rc, msg)
    message("info", debug.getinfo(1).source, debug.getinfo(1).currentline,
            "函数［ yun_heal_return ］参数：rc = "..tostring(rc)..", msg = "..tostring(msg))
    if var.yun_heal == nil then
        return rc,msg
    end
    var.yun_heal = nil
...
function yun_heal_exec()
    ...
    local l = wait_line("yun heal",
                        300, {StopEval=true}, 10, "^你并未受到内伤。$|"..
                        "^战斗中运功疗伤？找死吗？$|"..
                        "^你现在正忙着呢。$|"..
                        "^\\( 你上一个动作还没有完成，不能施用内功。\\)$|"..
                        "^在坐骑上运功疗伤，会走火入魔。$|"..
...
function take_drugs_exec(drug)
    ...
    local l = wait_line("get "..string.split(drug, ":")[2].." from "..msg,
                        30, nil, 10,
                        "^你从\\S+中拿出\\S+。$|"..
                        "^你找不到 \\S+ 这样东西。$|"..
                        "^你附近没有这样东西。$|"..
...
function ask_ping_return(rc, msg)
    message("info", debug.getinfo(1).source, debug.getinfo(1).currentline,
            "函数［ ask_ping_return ］参数：rc = "..tostring(rc)..", msg = "..tostring(msg))
    if var.ask_ping == nil then
        return rc,msg
    end
    var.ask_ping = nil
...
function force_recover_return(rc, msg)
    message("info", debug.getinfo(1).source, debug.getinfo(1).currentline,
            "函数［ force_recover_return ］参数：rc = "..tostring(rc)..", msg = "..tostring(msg))
    if var.force_recover == nil then
        return rc,msg
    end
...
function force_regenerate_return(rc, msg)
    message("info", debug.getinfo(1).source, debug.getinfo(1).currentline,
            "函数［ force_regenerate_return ］参数：rc = "..tostring(rc)..", msg = "..tostring(msg))
    if var.force_regenerate == nil then
        return rc,msg
    end
...
function dulong_yun_powerup()
    ...
    local l = wait_line("yun powerup",
                        30, nil, 10,
                        "^只见你高声叫道：“洪教主万年不老，永享仙福，寿与天齐！”$|"..
                        "^你已经在运功中了。$|"..
                        "^你的内力不够。$|"..
                        "^你已经受伤过重，只怕一运真气便有生命危险！$")
    if l == false then
...
function dulong_yun_cure()
    ...
    local l = wait_line("yun cure",
                        30, nil, 10,
                        "^你默念神龙口诀，将手指划破，开始凝神疗精。$|"..
                        "^你现在不须要疗精！$|"..
                        "^你的内力不够。$")
    if l == false then
        return -1
...
function heal_regenerate(pct, layer)
    message("info", debug.getinfo(1).source, debug.getinfo(1).currentline,
            "函数［ heal_regenerate ］参数：pct = "..tostring(pct)..", layer = "..tostring(layer))
...
function heal_regenerate_return(rc, msg)
    message("info", debug.getinfo(1).source, debug.getinfo(1).currentline,
            "函数［ heal_regenerate_return ］参数：rc = "..tostring(rc)..", msg = "..tostring(msg))
    if var.heal_regenerate == nil then
        return rc,msg
    end
...
function heal_recover(pct, layer)
    message("info", debug.getinfo(1).source, debug.getinfo(1).currentline,
            "函数［ heal_recover ］参数：pct = "..tostring(pct)..", layer = "..tostring(layer))
...
function heal_recover_return(rc, msg)
    message("info", debug.getinfo(1).source, debug.getinfo(1).currentline,
            "函数［ heal_recover_return ］参数：rc = "..tostring(rc)..", msg = "..tostring(msg))
    if var.heal_recover == nil then
        return rc,msg
    end
...
function dazuo_exec(target)
    ...
    local l = wait_line("dazuo "..tostring(var.dazuo.param[1]),
                        300, nil, 10,
                        "^你运功完毕，深深吸了口气，站了起来。$|"..
                        "^你现在正忙着呢。$|"..
