...
function zuanyan_return(rc)
    message("trace", debug.getinfo(1).source, debug.getinfo(1).currentline, "函数［ zuanyan_return ］参数：rc = "..tostring(rc))
    var.zuanyan = nil
    _ = nil
    return rc
end
...
function zuanyan_refresh_hp(rc)
    message("info", debug.getinfo(1).source, debug.getinfo(1).currentline, "函数［ zuanyan_refresh_hp ］参数：rc = "..tostring(rc))
    if var.zuanyan == nil then
        return rc
    end
...
function zuanyan_num_i(i)
    message("info", debug.getinfo(1).source, debug.getinfo(1).currentline, "函数［ zuanyan_num_i ］参数：i = "..tostring(i))
...
function zuanyan_goto_place(place)
    message("info", debug.getinfo(1).source, debug.getinfo(1).currentline, "函数［ zuanyan_goto_place ］参数：place = "..tostring(place))
...
function zuanyan_prepare_state(js, qx)
    message("info", debug.getinfo(1).source, debug.getinfo(1).currentline, "函数［ zuanyan_prepare_state ］参数：js = "..tostring(js)..", qx = "..tostring(qx))
...
function zuanyan_exec(i)
    message("info", debug.getinfo(1).source, debug.getinfo(1).currentline, "函数［ zuanyan_exec ］参数：i = "..tostring(i))
    local l = wait_line("zuanyan "..config.zuanyan[i].." "..tostring(var.zuanyan.times), 30, nil, 30, "^你现在精神不够，无法进行钻研。$|"..
                                                                                                      "^你的潜能不够，没有办法再钻研了。$|"..
                                                                                                      "^你只能参悟本派的武功。$|"..
                                                                                                      "^你没有根底，无法钻研这门武功。$|"..
...
function lian_return(rc)
    message("trace", debug.getinfo(1).source, debug.getinfo(1).currentline, "函数［ lian_return ］参数：rc = "..tostring(rc))
    var.lian = nil
    _ = nil
    return rc
end
...
function lian_refresh_hp(rc)
    message("info", debug.getinfo(1).source, debug.getinfo(1).currentline, "函数［ lian_refresh_hp ］参数：rc = "..tostring(rc))
    if var.lian == nil then
        return rc
    end
...
function lian_num_i(i, j)
    message("info", debug.getinfo(1).source, debug.getinfo(1).currentline, "函数［ lian_num_i ］参数：i = "..tostring(i)..",参数：j = "..tostring(j))
...
function lian_prepare_state(jl, qx)
    message("info", debug.getinfo(1).source, debug.getinfo(1).currentline, "函数［ lian_prepare_state ］参数：jl = "..tostring(jl)..",参数：qx = "..tostring(qx))
...
function lian_exec(i, j)
    ...
    elseif string.match(l[0], "开始练习") then
        ...
        l = wait_line(nil, 300, nil, 30, "^你修习完毕，深深吸了一口气，闭目而(?:坐|立)。$|"..
                                         "^你硬生生收回招式，不继续练习了。$|"..
                                         "^你硬生生将内息压回丹田，不继续修习了。$|"..
